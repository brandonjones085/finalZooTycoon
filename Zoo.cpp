...
void Zoo::babyBorn()
{
	srand(time(NULL));
	bool adult = false; 
	int num = std::rand() % 3 + 1; //Returns a random number between 1 and 3. 

	if (num == 1)//Tiger had baby
	{
		for (int i = 0; i < numOfTiger; i++)
		{
		...
		if (adult == true)
		{
			std::cout << "\n\nOne baby Tiger was born\n";
			addTiger(); 
		}
		else
		{
			std::cout << "\n\nNo Tigers are old enough\n\n";
			
			
		}
		
	}
	else if (num == 2)//Penguin had baby
	{
		for (int i = 0; i < numOfPenguin; i++)
		{
		...
		{
			std::cout << "\n\nNo penguins are old enough\n\n";
			
		}

		
	}
	else //Turtles had babye
	{
		for (int i = 0; i < numOfTurtle; i++)
		{
